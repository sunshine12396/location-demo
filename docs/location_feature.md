...
* `locations`: Stores primary location data, excluding localized names. This includes external IDs, types, geographic coordinates, hierarchical relationships.
...
The application calls the backend's **Autocomplete API** (`GET /locations/autocomplete`), which proxies to Google Places Autocomplete. This provides fast, type-ahead suggestions with `place_id`.

**Step 2: User Selects a Suggestion to Create a Post**  
When the user creates a post:
1. The frontend sends the `external_id` (Google Place ID) to the `POST /api/v1/posts` endpoint.
2. The backend checks if a location with this `external_id` exists locally.
3. **Just-in-Time Hydration**: If the location is missing or **outdated** (based on `LOCATION_SYNC_DAYS` config, e.g., > 30 days), the backend fetches full details (lat/lng, address components, translations) from Google and saves/updates the local record before linking the post.

**Step 3: Background Maintenance (Auto-Sync)**  
Whenever a location is viewed via `GetByID`, the system identifies if the data is stale. If so, it triggers a **background goroutine** to re-sync with Google, ensuring local data remains accurate without blocking the user request.
...
* **Standardized Location Types:** Implement a comprehensive set of location types (`country`, `city`, `district`, `landmark`, `venue`, `address`) for better categorization and mapping with Google primary types.
...
**What is completed:**
1. **Core Database Schema**: All tables (`locations`, `location_translations`, `location_alias`, `posts`, `location_stats`, `trending_locations`) are fully implemented with `updated_at` tracking for cache invalidation.
2. **Google Places Integration**: The backend is standardized on **Google Places API (New)**. It implements Text Search, Autocomplete, and Place Details.
3. **Intelligent Hydration & Sync**:
    *   **Waterfall Search**: Local Alias → Local Translation → External Search.
    *   **Background Sync**: Automated data refreshing for outdated locations.
    *   **Post Hydration**: Instant creation-time hydration for locations selected via Autocomplete.
4. **Hierarchical Graph**: Navigation seamlessly uses `parent_id` and `path` for breadcrumbs and localized post filtering.
5. **Interactive UI**: A Next.js frontend with live search, autocomplete suggestions, and post creation.
